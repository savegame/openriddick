...
- **Исходники в UTF-8** (конвертированы из CP1252 скриптом `Tools/convert_to_utf8.sh`, 2026-08-03). Часть файлов с CRLF — сохранять окончания строк как в файле, массово не переделывать.
...
- **СВЕРЯТЬСЯ С ЭТАЛОНОМ ДО ПРАВКИ, а не после.** Порядок источников, от
  самого надёжного: (1) шипнутые данные игры — `shaders/VP.xrg`,
  `shaders/ARB_Fragment_Program/*.fp`, `shaders/HL_Shading/*.fp`: это
  буквально то, что исполнял ретейл; (2) исходники движка в дереве — они
  часто содержат ответ прямо в функции, которая готовит данные для
  шейдера (`CreateProjMapTexGenAttr`, `PrepareFrame` и пр.); (3)
  декомпилы в корне (`RndrGL_dll_decomp.c` — PC-рендерер, `MXR_`,
  `MSystem_`, `GameClasses_`, `GameWorld_`) — только grep по строковым
  литералам, GL-константам (`0x8513` = GL_TEXTURE_CUBE_MAP и т.п.) и
  обратимым djb2-хэшам; (4) PS3-бэкенд `RenderContexts/PS3GCM/` как
  вторая реализация той же абстракции.
  Счёт этой сессии: каждая версия, проверенная по эталону, закрывалась
  за один заход (порядок компонент TSLV, CUBE и `.a` у проекции,
  спекулярная карта на юните 1); каждая версия «по симптому» стоила
  прогона и один раз завела в тупик.
- **`port_status.md` в корне — файл статуса порта.** Там текущие задачи, очередь
  и кратко сделанное. Обновлять при каждой смене этапа: новая задача — в
  «Сейчас в работе» или «Очередь», завершённая — строкой в «Сделано» с датой и
  одним-двумя предложениями сути. Подробные разборы по-прежнему в `Docs/`
  (спеки/отчёты), карта хаков и env-флагов — в `Docs/HacksAndHooks.md`.
...
- Полезные env-переменные порта:
  ...
  - **Шейдерный конвейер — ДЕФОЛТ ON с 2026-08-05** (`Docs/HacksAndHooks.md`
    «Дефолты шейдерного конвейера»): `RIDDICK_FP20`, `RIDDICK_NDS`,
    `RIDDICK_LFM` включены по умолчанию, каждый выключается своим `=0`.
    В строке запуска их указывать больше не нужно. Работают все пять
    forward-программ (LFM/LF/NDS/NDSP/NDSEATP), caps объявляют FP20 и
    8 текстурных юнитов, `XR_SHADERMODE` пиниться в 5.
  ...
  - `RIDDICK_HWSKIN` — **ДЕФОЛТ ON с 2026-08-04** (`=0` вернуть CPU-скиннинг):
    без него скиннед-геометрия идёт по streamed-пути без тангентов, все
    per-light программы её отвергают и персонажи рисуются плоским
    аддитивным диффузом. Прогон с ним: `fpfb=0`, `skin=734`.
    (2026-07-30, подробно — `Docs/HacksAndHooks.md`
    «RIDDICK_HWSKIN»): объявить `CRC_CAPS_FLAGS_MATRIXPALETTE` и увести
    движок на аппаратный скиннинг (анимированные кластеры идут VBID +
    палитра вместо CPU-трансформации в VB-арену). Подразумевает
    `RIDDICK_SKINNING=1`. Обоснование измерено: `[MP]` даёт 100%
  ...
  - Скиннинг и «растянутый через экран полигон» (2026-07-30, подробно —
    `Docs/HacksAndHooks.md` «Скиннинг: палитра и …»): скиннед-формат без
    палитры считается (`nopal=`) и логируется, но по-прежнему рисуется —
    у не анимированного кластера это штатная bind-поза; выбросить такие
    draw'ы для A/B — `RIDDICK_SKIN_DROPNOPALETTE=1`;
...
- `Docs/ResourceLists/ps3_files.txt` — дерево распакованного образа PS3.
  Использовать как СПРАВОЧНИК форматов/состава: PS3-ресурсы местами в
  другой упаковке (напр. диалоги там одним `ALL.XCD`, на PC — россыпью
  `.xrg` по подпапкам).

  **Поправка 2026-08-08: утверждение «PS3-ресурсы big-endian» ОШИБОЧНО**,
  здесь оно стояло по предположению. Прогон нашего LE-бинаря прямо по
  дереву `PS3_GAME/USRDIR` прочитал 3510 поверхностей, шрифты, реестры и
  XDF-архивы, отрисовал текстуры корректных размеров и вывел на экран
  строку сборки из PS3-контента (`GMC-1.142 … changelist 155238`). На
  big-endian данных не прошло бы ничего из этого. Это согласуется с
  моделью движка: `SwapLE()` — no-op на LE и байтсвоп на BE
  (`MMisc.h:638/655`), то есть **формат файла определён как
  little-endian**, а PS3-сборка свопила при загрузке.
...
`Docs/Decomp_Map.md` — единственное место, где ведутся соответствия
`FUN_xxxxxxxx` из Ghidra-декомпилов ↔ функции исходников. Опознал функцию
при разборе — впиши туда вместе с доказательством (строковый литерал,
порядок полевых чтений, уникальная константа). Догадки не писать: если
соответствие частичное, пометить явно.

Там же копится список известных расхождений между этим PS3-снапшотом
исходников и шипнутым PC-билдом — их надо учитывать при любой сверке.
