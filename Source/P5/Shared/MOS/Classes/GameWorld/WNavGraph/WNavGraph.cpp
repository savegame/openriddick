...
void CXR_NavGraph_Builder::SortSizeGroups(int _Sizes, TList_Vector<int>* _pRes, int _iExcludeSize)
{
	if (!_pRes)
		return;
...
void CXR_NavGraph_Builder::CheckEdgeSizes(CNB_Pathfinder * _pPathFinder, CConstructionEdge * _pEdge, int _nSizes, const int * _lSizes)
{
	...
	TList_Vector<CVec3Dint16> CachedPath;
...
void CXR_NavGraph_Builder::BuildGraph(CWorld_BlockNav * _pBlockNav, CWorld_Server * _pServer, const CVec3Dint16& _Start, int _iMinDist, int _Sizes)
{
	...
	TProfileDef(T);//DEBUG
	{
		...
		TProfileDef(T_Nodes);
		{
			...
			{
				Start = Search.FindStartPosition(_pServer, iMinstart, MinStart);
			}
			...
			while (CurCheckLayer < nCheckLayers)
			{
				while (Start != CVec3Dint16(-1))
				{
					...
					//Place node at start position and mark it as visited
		...
		int nCulled = 0;
		{
			...
			int VeryCloseSqr = Sqr(m_MinDist + m_MinDist / 2);
			int iDistSqr;
				...
				for (i = 0; i < m_lCNodes.Len(); i++)
				{
					...
					for (int j = 1; j < m_lCNodes[i].NumEdges(); j++)
					{
						if (m_lCEdges.ValidPos(m_lCNodes[i].m_lEdges[j]))
						{
							...
							if(HasEdgePathCostLimit(&Search, i, pEdge->m_iTo, pEdge->m_Cost * CostFactor, pEdge->m_Cost - 1))
							{
								//Cull edge
			...
			if (_Sizes & ~(1 << CXR_NavGraph::SIZE_DEFAULT))
			{
				...
				LogFile(CStr("(NavGraph) Additional Size Groups:"));
				{
					...
					{
						int BaseSize = (CXR_NavGraph::GetBaseSize(lSizeGroups[i]) * 2 + 1) * _pBlockNav->UnitsPerCell();
						int Height = CXR_NavGraph::GetHeight(lSizeGroups[i]) * _pBlockNav->UnitsPerCell();
		...
		LogFile(CStrF("(NavGraph) Copied graph (%f seconds).", T_Build));
...
//Debug render graph
void CXR_NavGraph_Builder::DebugRender(CWorld_Server * _pServer)
{
	MAUTOSTRIP(CXR_NavGraph_Builder_DebugRender, MAUTOSTRIP_VOID);
	if (_pServer && _pServer->Path_GetBlockNav())
	{
		int i;
		for (i = 0; i < m_lNodes.Len(); i++)
		{
			CVec3Dfp32 Wpos = _pServer->Path_GetBlockNav()->GetWorldPosition(m_lNodes[i].m_GridPos);
			_pServer->Debug_RenderWire(Wpos, Wpos + CVec3Dfp32(0,0,50), 0xffffff00, 1000);
		}
		for (i = 0; i < m_lEdges.Len(); i++)
		{
			CVec3Dfp32 WFrom = _pServer->Path_GetBlockNav()->GetWorldPosition(m_lNodes[m_lEdges[i].m_iFrom].m_GridPos);
			CVec3Dfp32 WTo = _pServer->Path_GetBlockNav()->GetWorldPosition(m_lNodes[m_lEdges[i].m_iTo].m_GridPos);
			_pServer->Debug_RenderWire(WFrom + CVec3Dfp32(0,0,50), WTo, 0xff00ff00, 1000);
		}
	}
};
...
void CXR_NavGraph_Builder::CNB_Pathfinder::Create(CWorld_BlockNav* _pBlockNav, int _TileHeap, int _MaxNodes, int _iBaseSize, int _iHeight)
{
	MAUTOSTRIP(CXR_NavGraph_Builder_CNB_Pathfinder_Create, MAUTOSTRIP_VOID);
	CWBN_SearchInstance::Create((_pBlockNav) ? _pBlockNav->GetGrid() : NULL, _pBlockNav, _TileHeap, _MaxNodes);
...
bool CXR_NavGraph_Builder::CNB_Pathfinder::CanPrune(const CVec3Dint16& _Pos)
{
	...
	const CWBN_SearchNode* pNodeCell = Hash_GetNode(_Pos[0], _Pos[1], _Pos[2]);
	if (pNodeCell)
	{
...
void CXR_NavGraph_Builder::CNB_Pathfinder::VisitNode(CWBN_SearchNode* _pN)
{
...
CVec3Dint16 CXR_NavGraph_Builder::CNB_Pathfinder::FindStartPosition(CWorld_Server * _pServer, int& _iMinObject, CVec3Dint16& _MinStart)
{
	MAUTOSTRIP(CXR_NavGraph_Builder_CNB_Pathfinder_FindStartPosition, CVec3Dint16(0,0,0));
	CVec3Dint16 Res = CVec3Dint16(-1);
	if ((m_nObjects == -1) || (_iMinObject < m_nObjects))
	{
		//Try to find position by searching through objects
		Res = FindStartPosition(_pServer, _iMinObject);
	}

	if (Res != CVec3Dint16(-1))
	{
		return Res;
	}
	else
	{
		//Scan over entire layer
		int LayerHeight = (m_pCheck) ? m_pCheck->GetLayerTop() - m_pCheck->GetLayerBottom() : 1000;
		return FindStartPosition(_MinStart, LayerHeight, m_ObjectBoundMax, m_ObjectBoundMin);
	}
};
...
CVec3Dint16 CXR_NavGraph_Builder::CNB_Pathfinder::FindStartPosition(CVec3Dint16& _MinStart, int16 _CheckHeight, const CVec3Dint16& _TightCheckMax, const CVec3Dint16& _TightCheckMin)
{
	MAUTOSTRIP(CXR_NavGraph_Builder_CNB_Pathfinder_FindStartPosition, CVec3Dint16(0,0,0));
	if (!m_pCheck)
		return CVec3Dint16(-1);	
	...
	//DEBUG
	//if (m_pServer && m_pServer->Path_GetBlockNav())
	//{
	//	ConOutL(CStrF("MinStart: %d,%d,%d (", _MinStart[0], _MinStart[1], _MinStart[2]) + m_pServer->Path_GetBlockNav()->GetWorldPosition(_MinStart).GetString() +  ")");
	//	m_pServer->Debug_RenderWire(m_pServer->Path_GetBlockNav()->GetWorldPosition(_MinStart), CVec3Dfp32(0), 0xff00ff00, 1000);
	//}
	//DEBUG
	...
	int x,y,z;
	for (z = StartZ; z >= StopZ; z -= (((z <= _TightCheckMax[2] + 7) && (z >= _TightCheckMin[2] - 7)) ? 1 : 8))
	{
		...
		for (y = YStart; y < m_pBlockNav->GridDimensions()[1] - m_iBaseSize - 1; y += (((y <= _TightCheckMax[1] + 31) && (y >= _TightCheckMin[1] - 31)) ? 8 : 32))
		{
			//If we're still on first row then use minstart value
			int XStart = (bFirstRow) ? Max((int)_MinStart[0], (int)m_iBaseSize) : m_iBaseSize;
			for (x = XStart; x < m_pBlockNav->GridDimensions()[0] - m_iBaseSize - 1; x += (((x <= _TightCheckMax[0] + 31) && (x >= _TightCheckMin[0] - 31)) ? 8 : 32))
			{
				Pos = CVec3Dint16(x, y, z);

				//DEBUG
				//ConOutL(CStrF("Check Pos: %d, %d, %d", x,y,z));
				//if ((x % 10 == 0) && (y % 10 == 0) && (z % 10 == 0) && m_pServer && m_pServer->Path_GetBlockNav())
				//{
				//	CVec3Dfp32 p = m_pServer->Path_GetBlockNav()->GetWorldPosition(Pos);
				//	m_pServer->Debug_RenderWire(p, p + CVec3Dfp32(0,0,4), 0xff00ff00, 1000);
				//}
				//DEBUG
...
//Finds traversable ground position that hasn't been visited yet by exploring the space around all objects 
//in the given server. This is of course incomplete sometimes
CVec3Dint16 CXR_NavGraph_Builder::CNB_Pathfinder::FindStartPosition(CWorld_Server * _pServer, int& _iMinObject)
{
	MAUTOSTRIP(CXR_NavGraph_Builder_CNB_Pathfinder, CVec3Dint16(0,0,0));

	//Do we have to find objects?
	if (m_nObjects == -1)
	{
		//Select all objects in the world and search positions around each one, 
		//starting at given min index until a suitable start position has been found
		int iSel;
		...
		{
			//Since we couldn't set up a selection we can't find any object positions
			return CVec3Dint16(-1);
		};
		
		//Add all objects to selection, get selection result and free selection
		_pServer->Selection_AddAll(Selection);
		nObjects = _pServer->Selection_Get(Selection, &piObjects);
		_pServer->Selection_Pop();

		//Add those in layer to objects array, and find object bounds within layer
		CWObject * pObj;
		CVec3Dfp32 ObjBoundMax = CVec3Dfp32(-_FP32_MAX);
		CVec3Dfp32 ObjBoundMin = CVec3Dfp32(_FP32_MAX);
		fp32 LayerBottom = m_pBlockNav->GetWorldPosition(CVec3Dint16(0, 0, m_pCheck->GetLayerBottom()))[2];
		fp32 LayerTop = m_pBlockNav->GetWorldPosition(CVec3Dint16(0, 0, m_pCheck->GetLayerTop()))[2];
		for (int i = 0; i < nObjects; i++)
		{
			pObj = _pServer->Object_Get(*piObjects);
			if (pObj &&
				(pObj->GetPosition()[2] <= LayerTop) &&
				(pObj->GetPosition()[2] >= LayerBottom))
			{
				m_lObjects.Add(pObj->m_iObject);
				
				for (int j = 0; j < 3; j++)
				{
					if (pObj->GetPosition()[j] > ObjBoundMax[j])
						ObjBoundMax[j] = pObj->GetPosition()[j];
					if (pObj->GetPosition()[j] < ObjBoundMin[j])
						ObjBoundMin[j] = pObj->GetPosition()[j];
				}
			}
			piObjects++;
		}
		m_nObjects = m_lObjects.Len();
		m_ObjectBoundMax = m_pBlockNav->GetGridPosition(ObjBoundMax) + CVec3Dint16(8,8,8);
		m_ObjectBoundMin = m_pBlockNav->GetGridPosition(ObjBoundMin) - CVec3Dint16(8,8,8);
	}

	//Iterate over objects, starting at given index
	CVec3Dint16 Res = CVec3Dint16(-1);
	CWObject * pObj;
	CVec3Dint16 GridPos;
	for (int i = _iMinObject; i < m_lObjects.Len(); i++)
	{
		pObj = _pServer->Object_Get(m_lObjects[i]);
		if (pObj)
		{
			GridPos = m_pBlockNav->GetGridPosition(pObj->GetPosition());
			Res = FindPathPosition(GridPos, 5);
			if (Res != CVec3Dint16(-1))
			{
				//Found position!
				_iMinObject = i;
...
int CXR_NavGraph_Builder::CNB_Pathfinder::StraightPath(const CVec3Dint16& _From, const CVec3Dint16& _To)
{
	...
	if (m_pBlockNav->IsGroundTraversableImpl(_From,_To, m_iBaseSize, m_iHeight, 1, 1))
	{
		...
		return WBN_DISTANCEFACTOR * Length3(WBN_COSTFRACTIONS * (_From[0] - _To[0]), WBN_COSTFRACTIONS * (_From[1] - _To[1]), WBN_COSTFRACTIONS * (_From[2] - _To[2]));
	}
	else
	{
		return 0;
...
int CXR_NavGraph_Builder::CNB_Pathfinder::BreadthFirstFindPositions(const CVec3Dint16& _StartPos, int _iDepth, TThinArray<CPosCost> * _plResult)
{
	...
	_iDepth *= WBN_COSTFRACTIONS * WBN_DISTANCEFACTOR; //Convert node-depth to depth in world units
...
int CXR_NavGraph_Builder::CNB_Pathfinder::AStarFindEdge(const CVec3Dint16& _From, const CVec3Dint16& _To, int _iGroundDist, int _iJumpDist, int _iFallDist, int _iGroundTail, int& _iCost, int _BaseSize, int _Height, TList_Vector<CVec3Dint16> * _plResult)
{
	...
	int iMaxCost = _iGroundDist * 4 * WBN_COSTFRACTIONS * WBN_DISTANCEFACTOR;
...
{	
	//Check start
	if ((_nPos < 1) || !m_pBlockNav->IsTraversableImpl(_lPos[0], _BaseSize, _Height))
		return false;
	...
	{
		if (!m_pBlockNav->IsTraversableImpl(_lPos[i], _BaseSize, _Height, _lPos[i - 1][0] - _lPos[i][0], _lPos[i - 1][1] - _lPos[i][1], _lPos[i - 1][2] - _lPos[i][2]))
			return false;
	}
