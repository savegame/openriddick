...
void CWRes_Model_XW::OnLoad()
{
	...
	// Root cause of Pa1_TheDream Phys/P_ItemBox.xw:2 wild-pointer crash:
	// ReadAllModels could return false, or succeed but never set m_spModel
	// (when requested sub-model index doesn't exist in the .xw file — the
	// iModel==ModelNr branch on WDataRes_XW.cpp:362 never fires). Setting
	// WRESOURCE_LOADED unconditionally lied about the state, m_spModel
	// stayed default/re-used, GetModel() returned a wild pointer that later
	// crashed OnPrecache/CreatePhys. Now only mark LOADED when m_spModel is
	// actually populated.
	bool bOk = ReadAllModels(m_pWData, "CXR_Model_BSP");
	if (bOk && m_spModel != NULL)
		m_Flags |= WRESOURCE_LOADED;
	else
	{
		static int s_nWarn = 0;
		if (s_nWarn++ < 20)
			fprintf(stderr, "[WRES-XW] OnLoad('%s'): ReadAllModels %s, m_spModel=%p — leaving UNLOADED\n",
			        m_Name.Str(), bOk ? "ok" : "failed", (void*)(CXR_Model*)m_spModel);
	}
}
...
*/
	...
	// If we requested sub-model ModelNr but never matched it in the file
	// (fewer sub-models than expected), m_spModel is still default/wild —
	// signal to caller so it doesn't mark the resource LOADED.
	if (ModelNr >= nModels)
	{
		LogFile(CStrF("(CWRes_Model_XW::ReadAllModels) '%s': requested sub-model %d but file has only %d — skipping",
		              m_Name.Str(), ModelNr, nModels));
		return false;
	}
	return true;
}
...
void CWRes_Model_XW2::OnLoad()
{
	...
	bool bOk = ReadAllModels(m_pWData, "CXR_Model_BSP2");
	if (bOk && m_spModel != NULL)
		m_Flags |= WRESOURCE_LOADED;
	else
	{
		static int s_nWarn = 0;
		if (s_nWarn++ < 20)
			fprintf(stderr, "[WRES-XW2] OnLoad('%s'): ReadAllModels %s, m_spModel=%p — leaving UNLOADED\n",
			        m_Name.Str(), bOk ? "ok" : "failed", (void*)(CXR_Model*)m_spModel);
	}
}
...
void CWRes_Model_XW3::OnLoad()
{
	...
	bool bOk = ReadAllModels(m_pWData, "CXR_Model_BSP3");
	if (bOk && m_spModel != NULL)
		m_Flags |= WRESOURCE_LOADED;
	else
	{
		static int s_nWarn = 0;
		if (s_nWarn++ < 20)
			fprintf(stderr, "[WRES-XW3] OnLoad('%s'): ReadAllModels %s, m_spModel=%p — leaving UNLOADED\n",
			        m_Name.Str(), bOk ? "ok" : "failed", (void*)(CXR_Model*)m_spModel);
	}
}
...
void CWRes_Model_XW4::OnLoad()
{
	...
	bool bOk = ReadAllModels(m_pWData, "CXR_Model_BSP4");
	if (bOk && m_spModel != NULL)
		m_Flags |= WRESOURCE_LOADED;
	else
	{
		static int s_nWarn = 0;
		if (s_nWarn++ < 20)
			fprintf(stderr, "[WRES-XW4] OnLoad('%s'): ReadAllModels %s, m_spModel=%p — leaving UNLOADED\n",
			        m_Name.Str(), bOk ? "ok" : "failed", (void*)(CXR_Model*)m_spModel);
	}
}
...
void CWRes_Model_Glass::OnLoad()
{
	...
	bool bOk = ReadAllModels(m_pWData, "CXR_Model_BSP4Glass");
	if (bOk && m_spModel != NULL)
		m_Flags |= WRESOURCE_LOADED;
	else
	{
		static int s_nWarn = 0;
		if (s_nWarn++ < 20)
			fprintf(stderr, "[WRES-Glass] OnLoad('%s'): ReadAllModels %s, m_spModel=%p — leaving UNLOADED\n",
			        m_Name.Str(), bOk ? "ok" : "failed", (void*)(CXR_Model*)m_spModel);
	}
}
