...
\*************************************************************************************************/
// Same wild-pointer guard as WMapData.h GetResource_Model — some
// CWRes_Model_XW instances have m_spModel pointing at heap-reused memory
// (canonical VA but content is not a real object), virt call crashes.
// See WMapData.h for full analysis of the Pa1_TheDream case.
static inline bool IsPlausibleCXRModelPtr(const void* p)
{
	uintptr_t up = (uintptr_t)p;
	if ((up & 0x7) != 0 || up < 0x1000 || (up >> 47) != 0)
		return false;
	uintptr_t vtable = *(const uintptr_t *)p;
...
void CWRes_Model::OnPrecache(class CXR_Engine* _pEngine)
{
	...
	if (pModel && IsPlausibleCXRModelPtr(pModel))
		pModel->OnPrecache(_pEngine, 0);
	else if (pModel)
	{
		static int s_nWarn = 0;
		if (s_nWarn++ < 20)
			fprintf(stderr, "[WRES] CWRes_Model::OnPrecache: skipping bogus pModel=%p for '%s'\n",
			        (void*)pModel, m_Name.Str());
	}
}
