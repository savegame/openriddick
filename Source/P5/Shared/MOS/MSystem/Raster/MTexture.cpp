...
void CTextureCache::WriteCache(const char* _pFilename)
{
	...
	// КЭШ УМЕЕТ ТОЛЬКО СТАРЫЙ КЛАСС КОНТЕЙНЕРА.
	//
	// И писатель ниже, и `ReadCache` выше знают ровно один класс --
	// `VirtualXTC`. `VirtualXTC2` ему не наследник, а самостоятельный класс
	// (см. `CLAUDE.md`), поэтому `TDynamicCast` даёт NULL и дальше стоял
	// `Error_static` -- то есть у нас SIGILL, потому что исключения
	// выключены.
	//
	// Это не наш недосмотр в порте, а следствие авторской конфигурации:
	// вся функция под `#ifndef PLATFORM_CONSOLE`, а XTC2 -- КОНСОЛЬНЫЙ
	// формат. В ретейле писатель и XTC2 просто никогда не встречались:
	// на PS3 кэш не пишется вовсе. У нас же Linux -- не консоль, и они
	// встретились впервые.
	//
	// Поэтому: не выдумываем сериализацию XTC2 (её нет и в ретейле, и
	// прочитать её `ReadCache` всё равно не смог бы), а просто не пишем
	// кэш, когда в наборе есть контейнер, который писателю незнаком.
	// Кэш -- это ускорение повторного скана, его отсутствие ничего не ломает.
	for (uint i = 0; i < m_lspTC.Len(); i++)
	{
		if (!TDynamicCast<CTextureContainer_VirtualXTC>((CTextureContainer*)m_lspTC[i]))
		{
			static bool bLogged = false;
			if (!bLogged)
			{
				bLogged = true;
				LogFile("(CTextureCache::WriteCache) В наборе есть контейнер не класса VirtualXTC "
					"(вероятно VirtualXTC2) -- кэш скана текстур не пишется. Это не ошибка: "
					"кэш умеет только старый класс, а на консольных сборках он не пишется вовсе.");
			}
			return;
		}
	}
