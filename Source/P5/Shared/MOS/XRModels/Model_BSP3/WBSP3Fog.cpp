...
// -------------------------------------------------------------------
int g_BSP3_nFogTraceNode = 0;
int g_BSP3_nFogTraceLeaf = 0;
int g_BSP3_nFogRays = 0;
int g_BSP3_nFogRayLamps = 0;
int g_BSP3_nLightIterSave = 0;
int g_BSP3_nLightIter = 0;
...
CBSP3_FogBox::CBSP3_FogBox()
{
	MAUTOSTRIP(CBSP3_FogBox_ctor, MAUTOSTRIP_VOID);
	m_BoxOrigin = 0;
	m_BoxScale = 0;
	m_BoxX = 0;
	m_BoxY = 0;
...
void CBSP3_FogBox::Create(const CBox3Dfp32& _Box, const CVec3Dfp32& _CellSize)
{
	MAUTOSTRIP(CBSP3_FogBox_Create, MAUTOSTRIP_VOID);
	CVec3Dfp32 Ext;
...
//ConOutL(CStrF("(CBSP3_FogBox::Create) %d, %d, %d", m_BoxX, m_BoxY, m_BoxZ));
}

/*void CBSP3_FogBox::Create(const CBSP3_LightVolumeInfo& _LightVol)
{
	MAUTOSTRIP(CBSP3_FogBox_Create_2, MAUTOSTRIP_VOID);
...
//ConOutL(CStrF("(CBSP3_FogBox::Create) Create from lightvolume, %d, %d, %d", m_BoxX, m_BoxY, m_BoxZ));
}*/

void CBSP3_FogBox::Clear()
{
	MAUTOSTRIP(CBSP3_FogBox_Clear, MAUTOSTRIP_VOID);
	FillChar(m_lBox.GetBasePtr(), m_lBox.ListSize(), 0);
}

void CBSP3_FogBox::AddLight(const CXR_Light* _pL)
{
	MAUTOSTRIP(CBSP3_FogBox_AddLight, MAUTOSTRIP_VOID);
...
void CBSP3_FogBox::SetAlphaFromBrightness()
{
	MAUTOSTRIP(CBSP3_FogBox_SetAlphaFromBrightness, MAUTOSTRIP_VOID);
	int nc = m_lBox.Len();
...
/*CPixel32 CBSP3_FogBox::Filter(const CPixel32* pC, int x, int y, int z)
{
	MAUTOSTRIP(CBSP3_FogBox_Filter, NULL);
}
*/

void CBSP3_FogBox::Filter()
{
	MAUTOSTRIP(CBSP3_FogBox_Filter_2, MAUTOSTRIP_VOID);
	CPixel32 Box[2048];
...
CPixel32 CBSP3_FogBox::GetFog(const CVec3Dfp32& _v, int _Alpha) const
{
	MAUTOSTRIP(CBSP3_FogBox_GetFog, CPixel32());
	CVec3Dfp32 v;
...
CPixel32 CBSP3_FogBox::GetFog_BoxSpace(const CVec3Dfp32& _v, int _Alpha) const
{
	MAUTOSTRIP(CBSP3_FogBox_GetFog_BoxSpace, CPixel32());
	CVec3Dint iv;
...
void CBSP3_FogBox::GetFogBoxVectors(const CVec3Dfp32& _p0, const CVec3Dfp32& _v, CVec3Dfp32& _fp0, CVec3Dfp32& _fv) const
{
	MAUTOSTRIP(CBSP3_FogBox_GetFogBoxVectors, MAUTOSTRIP_VOID);
	CVec3Dfp32 p;
...
//  CXR_Model_BSP3::Fog_...
// -------------------------------------------------------------------
void CXR_Model_BSP3::Fog_InitFogBox(int _iPL)
{
	MAUTOSTRIP(CXR_Model_BSP_Fog_InitFogBox, MAUTOSTRIP_VOID);
//	CBSP3_PortalLeafExt* pPL = &m_lPortalLeaves[_iPL];

/*	const CBSP3_LightVolumeInfo* pLightVol = (pPL->m_iLightVolume) ? &m_lLightVolumeInfo[pPL->m_iLightVolume] : NULL;
	if (!pLightVol) return;

	const CXR_MediumDesc* pMedium = &m_lMediums[pPL->m_iMedium];

	spCBSP3_FogBox spBox = DNew(CBSP3_FogBox) CBSP3_FogBox;
...
void CXR_Model_BSP3::Fog_AddLightVolume(CBSP3_FogBox* _pBox, int _iLightVolume)
{
	MAUTOSTRIP(CXR_Model_BSP_Fog_AddLightVolume, MAUTOSTRIP_VOID);
/*	const CBSP3_LightVolumeInfo* pLightVol = &m_lLightVolumeInfo[_iLightVolume];
	...
	while(pLightVol)
	{
		...
		{
			ConOut(CStrF("(CXR_Model_BSP3::Fog_AddLightVolume) Light-volume size missmatch. (%d != %d)", pLightVol->m_CellsPerGrid, _pBox->m_lBox.Len() ));
			return;
		}

//ConOut(CStrF("(CXR_Model_BSP3::Fog_AddLightVolume) Cells %d, ", pLightVol->m_CellsPerGrid));
...
void CXR_Model_BSP3::Fog_BuildFogBox(int _iPL)
{
	...
	CBSP3_FogBox* pBox = m_lspFogBoxes[pPL->m_iFogBox];
...
CPixel32 CXR_Model_BSP3::Fog_EvalFogBox(int _iPL, const CVec3Dfp32& _p0, const CVec3Dfp32& _p1, int _MaxAlpha)
{
...
//		g_FogTime = m_pCurAnimState->m_AnimTime0;
		CBSP3_FogBox* pFogBox = m_lspFogBoxes[pPL->m_iFogBox];
...
CPixel32 CXR_Model_BSP3::Fog_TracePortalLeaf(int _iFirstNode, int _iDestPL, const CVec3Dfp32& _p0, const CVec3Dfp32& _p1, int _MaxAlpha)
{
	...
	const CBSP3_PortalLeafExt* pPL = &m_lPortalLeaves[_iDestPL];
...
CPixel32 CXR_Model_BSP3::Fog_TraceLine_FogBox(int _iNode, const CVec3Dfp32& _p0, const CVec3Dfp32& _p1, int _MaxAlpha)
{
...
void CXR_Model_BSP3::Fog_TraceVertices(const uint32* _piFaces, int _nFaces, int _iPL)
{
	...
	g_BSP3_nFogTraceNode = 0;
	g_BSP3_nFogTraceLeaf = 0;
	g_BSP3_nLightIter = 0;
	g_BSP3_nLightIterSave = 0;
	g_BSP3_nFogRays = 0;
	g_BSP3_nFogRayLamps = 0;
...
//		bool bFog = false;
		for(int v = 0; v < nv; v++)
		{
			...
			{
g_BSP3_nFogRays++;
				CPixel32 F = Fog_TracePortalLeaf(iFirstNode, _iPL, m_pVertices[iv], m_pView->m_CurLocalVP, 255);
				m_lVertexFog[itv] = F;
...
//ConOut(CStrF("N/L %d, %d, Rays %d, Lights/Ray %.2f, LightIterSave %d / %d", g_BSP3_nFogTraceNode, g_BSP3_nFogTraceLeaf, g_BSP3_nFogRays, fp32(g_BSP3_nFogRayLamps)/fp32(g_BSP3_nFogRays), g_BSP3_nLightIterSave, g_BSP3_nLightIter));
...
void CXR_Model_BSP3::Fog_TraceVertices(const uint32* _piFaces, int _nFaces)
{
...
//		bool bFog = false;
		for(int v = 0; v < nv; v++)
		{
			...
			{
g_BSP3_nFogRays++;
				m_lVertexFog[itv] = ms_pSceneFog->Trace(m_pVertices[iv]);
...
void CXR_Model_BSP3::Fog_RenderPortal(int _iFirstNode, int _iPL, int _ip, int _iStartNode)
{
	MAUTOSTRIP(CXR_Model_BSP_Fog_RenderPortal, MAUTOSTRIP_VOID);
	const CBSP3_Portal* pP = &m_lPortals[_ip];
	if (!pP->m_iFogPortal) return;

	const CBSP3_FogPortal* pFP = &m_lFogPortals[pP->m_iFogPortal];
//ConOut(CStrF("   FogPortal %d, iP %d, PL %d", pP->m_iFogPortal, _ip, _iPL));
	...
	CVec2Dfp32* pTV = ms_pVBM->Alloc_V2(nV);
	CPixel32* pCol = ms_pVBM->Alloc_CPixel32(nV);
	uint16* pPrim = ms_pVBM->Alloc_Int16(nP);
	CRC_Attributes* pA = ms_pVBM->Alloc_Attrib();
	if (!pV || !pTV || !pCol || !pPrim || !pA) return;

	pA->SetDefault();
	if (ms_pCurrentEngine) ms_pCurrentEngine->SetDefaultAttrib(pA);
	pVB->m_pAttrib = pA;
	...
	pVB->m_Priority = ms_BasePriority_Opaque + CXR_VBPRIORITY_FOGPORTAL;
	pVB->Matrix_Set(ms_pVBMatrixM2V);

	pA->Attrib_TextureID(0, ms_pSceneFog->m_FogTableTextureID);
...
/*	if (_iFirstNode)
	{
		for(int v = 0; v < nv; v++)
			pCol[v] = Fog_TraceLine_FogBox(_iFirstNode, _iPL, m_pVertices[ivbase + v], m_pView->m_CurLocalVP, 255);
	}
	else*/
	{
		for(int v = 0; v < nv; v++)
			pCol[v] = Fog_TracePortalLeaf(_iFirstNode, _iPL, m_pVertices[ivbase + v], m_pView->m_CurLocalVP, 255);
	}

	if (!ms_pSceneFog->ConvertFogColors(nv, pCol, pTV))
		return;
...
//			pCol[v] = 0xff3f3f3f;
		...
		for(int f = 0; f < pFP->m_nFaces; f++)
		{
			const CBSP3_FogPortalFace* pFPF = &m_lFogPortalFaces[f + pFP->m_iFaces];
...
void CXR_Model_BSP3::Fog_RenderPortal_HW(int _iFirstNode_NotUsed_Remove, int _iPL, int _ip, int _iStartNode)
{
...
//	int nv = pFP->m_nVertices;
	...
	Fog_SetAttrib(pVB->m_pAttrib, pPL, m_pView->m_CurLocalVP);
...
int CXR_Model_BSP3::Fog_GetVisiblePortals(int _iPL, uint32* _piPortals, int _MaxPortals)
{
	...
	for(int p = 0; p < np; p++)
	{
		...
		const CBSP3_Portal* pP = &m_lPortals[ip];
		if (!pP->m_iFogPortal) continue;
		fp32 d = m_pPlanes[pP->m_iPlane].Distance(m_pView->m_CurLocalVP);
		...
		{
			ConOut("(CXR_Model_BSP3::Fog_GetVisiblePortals) Too many fog-portals.");
			continue;
		}
		_piPortals[nVisP++] = ip;
	}
...
void CXR_Model_BSP3::Fog_RenderPortals(int _iPL, uint32* _piPortals, int _nP)
{
...
bool CXR_Model_BSP3::Fog_SetVertices(CXR_VertexBuffer* pVB, const uint32* _piFaces, int _nFaces, const uint32* _piFaceVerts, int _iPLVBase)
{
	...
	fp32 oow = ms_pSceneFog->m_FogTableWidthInv;
	fp32 omw = ms_pSceneFog->m_FogTableWidth - 1;

	for(int f = 0; f < _nFaces; f++)
	{
		const CBSP3_Face* pF = &m_pFaces[_piFaces[f]];
		int nv = pF->m_nVertices;
		int iiv = pF->m_iiVertices;
...
bool CXR_Model_BSP3::Fog_SetAttrib(CRC_Attributes* _pAttrib, CBSP3_PortalLeafExt* _pPL, const CVec3Dfp32& _LocalEye)
{
	MAUTOSTRIP(CXR_Model_BSP_Fog_SetAttrib, false);
	ms_pSceneFog->SetDepthFogNone(_pAttrib);
...
//	_pAttrib->Attrib_AlphaCompare(CRC_COMPARE_ALWAYS, 0);
	_pAttrib->Attrib_TextureID(0, ms_pSceneFog->m_FogTableTextureID);

//	_pAttrib->Attrib_TextureID(0, ms_pCurrentEngine->m_pTC->GetTextureID("DM01_10"));

	CVec4Dfp32* pParams = ms_pVBM->Alloc_V4(3);
	if (!pParams)
		return false;
	...
	if (!(Medium.m_MediumFlags & XW_MEDIUM_FOG))
		ConOut("§cf80WARNING: (CXR_Model_BSP3::Fog_SetAttrib) Not a fog PL.");
...
// -------------------------------------------------------------------
void CXR_Model_BSP3::Fog_Trace(int _hAccelerator, const CVec3Dfp32& _POV, const CVec3Dfp32* _pV, int _nV, CPixel32* _pFog)
{
...
void CXR_Model_BSP3::Fog_EnableFogPL_r(int _iNode, const CBox3Dfp32& _Box)
{
...
void CXR_Model_BSP3::Fog_DisableFogPL_r(int _iNode, const CBox3Dfp32& _Box)
{
...
int CXR_Model_BSP3::Fog_InitTraceBound(const CBox3Dfp32& _Box)
{
...
void CXR_Model_BSP3::Fog_ReleaseTraceBound(int _hAccelerator)
{
	MAUTOSTRIP(CXR_Model_BSP_Fog_ReleaseTraceBound, MAUTOSTRIP_VOID);
	Fog_DisableFogPL_r(_hAccelerator, ms_Fog_LastBox);
