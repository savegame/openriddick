...
void CStream_XDF::Fallback_Read(void* dest, mint _Size)
{
	...
	// ЕСЛИ ФАЙЛА НЕТ НА ДИСКЕ -- ЧЕСТНО БРОСАЕМ, А НЕ ВЫДАЁМ НУЛИ.
	//
	// История правки. Пока исключения были выключены, неудачное открытие
	// внутри `INL_OpenFallbackFile` разворачивалось в `M_BREAKPOINT` -> SIGILL,
	// и я поставил здесь заглушку: вернуть нули и жить дальше. Она сняла
	// падение, но создала ХУДШУЮ болезнь -- чтение стало МОЛЧА УСПЕШНЫМ с
	// мусором. На `content\xdf\pa1_arrival.rul` это дало счётчик ресурсов из
	// нулей и практически бесконечный цикл: лог рос на десятки тысяч строк,
	// игра висела.
	//
	// Теперь исключения включены, поэтому правильная реакция -- та, что и
	// задумана движком: сообщить об ошибке. Наверху её ловит загрузка мира,
	// печатает отчёт и корректно сворачивается. Тихо подсовывать нули
	// нельзя: цикл, длина массива или размер блока, прочитанные из мусора,
	// уводят движок куда угодно.
	if (!m_spFallbackStream && !MRTC_SystemInfo::OS_FileExists(m_FileName.Str()))
	{
		...
		if (s_nWarn < 20)
		{
			...
			M_TRACEALWAYS("[XDF] запасное чтение невозможно: файла '%s' нет на диске "
				"(нужно %d байт с позиции %d)\n",
				m_FileName.Str(), (int)_Size, (int)m_FilePos);
		}
		FileError("Fallback_Read", m_FileName, 0);
	}
#endif
