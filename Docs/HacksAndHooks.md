...
- **HACK/FIX** `WAG2I_StateInst.cpp`, `EnterState_AdaptiveTimeScale`.
  ...
  Теперь ноль трактуется как «не задано» и длина пишется в свойство 8.
  `RIDDICK_ADAPTPROP=<n>` меняет номер, `=-1` возвращает прежнее поведение
  для A/B. Зонд печатает `prop=8(raw 0)`.

  NB: побочный эффект старого поведения — длина писалась поверх свойства 0,
  то есть `PROPERTY_FLOAT_STATETIME`. Условия переходов и таймауты, которые
  на него смотрят, получали 44…117 вместо времени состояния. Вероятная
  причина «NPC не атакует».
