...
- **DBG** `[ANIM]` (`WObj_CharAnim.cpp:360`, `Char_GetAnimLayers`, под
  ...
  периодическую печать (каждый 30-й вызов, кап 40 строк на объект) и
  дополнен двумя величинами:
  ...
  (искать в AG2/подаче времени); `v>0, dt>0` — слой живой, и дефект ниже
  по конвейеру (палитра/скиннинг/рендер).
