...
- **KEEP** `AnimGraph2_AnimLayer.cpp`, ветка v5/v6 — третий float записи
  анимслоя **не является opacity** в PC-формате, поэтому при нулевом
  значении восстанавливается дефолт (1.0).
  Доказательство (дамп `Pa1_Pit`, 2026-07-30): у всех выборочных слоёв
  `ofs=0.000 scale=1.000..1.500 opacity=0.000`, при этом `iAnim`
  (453..875), `base`, `merge`, `timeCtrl` — осмысленные, а шаг записи
  совпадает с файлом точно (18 байт, остаток 0). Значит соседние поля
  разложены верно и третий float действительно нулевой в данных. Быть
  opacity он не может: `CWAG2I_StateInstance::GetAnimLayers` считает
  `LayerBlend = Blend * GetOpacity()` и выбрасывает слой при нулевом
  произведении — контент с нулевой opacity везде не анимировался бы и на
  PC. Следствие использования его как opacity: все слои отброшены →
  `CXR_Skeleton::EvalAnim` не находит полнотелого слоя → обрывается и
  заливает все 120 костей QNaN → камера падает к ступням.
  Слой, заавторенный ровно с нулевой opacity, всё равно ничего не вносит
  (его выбрасывает тот же множитель), так что при восстановлении дефолта
  ничего осмысленного не теряется.
