...
- **DBG** `RIDDICK_SCISSOR_MIRRORX=1` (`ApplyAttribs`) — зеркалит
  scissor-прямоугольник по вертикальной оси при применении
  (`x0' = ScreenW - x1`). Диагностика под «левая часть кадра чёрная».
  Основание — лог `[GL-VIEW]` от 2026-07-29: вьюпорт, FBO и окно
  полностью согласованы (`1280x720`, `vp=(0,0..1280,720)`, `glY=0`,
  прямоугольник clear пустой = полная очистка цели), вырожденных
  прямоугольников нет (`empty=0`) — но ОБЪЕДИНЕНИЕ всех применённых за
  интервал scissor'ов раз за разом лежит в узкой правой полосе, например
  `union=(1088,0..1280,720)`, причём `maxX` ВСЕГДА ровно равен ширине
  экрана. Это подпись прямоугольников, отражённых относительно
  вертикальной оси: то, что должно быть слева, обрезается вправо, а слева
  остаётся чёрное.
  Подозреваемый источник — костыль с негированием X у view-матрицы
  (`XREngine.cpp`, X-столбец `m_W2VMat`): картинка и спроецированный
  scissor-бокс оказываются в разной хиральности. Если флаг чинит картинку,
