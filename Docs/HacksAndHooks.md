...
### Проблемы освещения (со слов пользователя, 2026-07-30, НЕ исследовано)

Записано как задача, отдельная от скиннинга (и от неё же зависит, насколько
можно верить виду скиннед-персонажей при включённых световых программах):

* фонарики персонажей, лампы и вообще всё, что накладывает текстуру поверх
  освещённой поверхности (круглый ореол фонарика, маска светового пятна),
  **затайливается**: вместо одного круга получается регулярная сетка
  повторяющихся кругов по всей освещённой поверхности.

Первый кандидат, проверять с него: у нас **wrap-режим захардкожен в
`GL_REPEAT` для всех текстур** (`GLES3_Texture.cpp:225-226`, `:359-360`,
раздел «Wrap mode = REPEAT» ниже) — это осознанный костыль bring-up'а ради
мировых тайлящихся текстур. Но у движка есть per-texture признак:
`CTC_TEXTUREFLAGS_CLAMP_U` / `CTC_TEXTUREFLAGS_CLAMP_V`
(`MSystem/Raster/MTexture.h:98-99`), доступный через
`CTextureContext::GetTextureFlags(int _ID)` (`MTexture.h:364`). Проекционные
маски света — ровно тот случай, для которого этот флаг и существует: за
пределами [0..1] должен быть край, а не следующая копия. Второй кандидат —
режим наложения UV у проекционного texgen (`CRC_TEXGENMODE_*`
для проекторов) и отсутствие у нас clamp-to-border.
