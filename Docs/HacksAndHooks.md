...
1. **Скиннед-геометрия без палитры.** Позиции в matrix-palette вершинном
   буфере — **костно-локальные**. Без палитры их нельзя нарисовать вообще:
   каждый кусок садится туда, куда попадает начало его кости во view-space.
   Раньше такой draw уходил в GL как есть (uBoneCount=0 → passthrough).
   Теперь **draw отбрасывается**; `RIDDICK_SKIN_DRAWNOPALETTE=1` возвращает
   старое поведение для A/B. Счётчик — `nopal=N` в `[GL-DBG]`, плюс
   безфлаговая строка `[GLES3-SKIN] no palette for a skinned draw (#N)` на
   1/100/1000/10000-м случае (одноразовая строка не отличала «одна шальная
   отрисовка» от «каждый персонаж каждый кадр»).
   Откуда берётся: палитру ставит `Cluster_SetMatrixPalette`
   (`WTriMesh.cpp:1106`), а зовут её только когда модели есть что дать
   (`pMatrixPalette != NULL`, `WTriMesh.cpp:5852`) — и она выходит ни с чем,
   когда VB-арена исчерпана. Так что растущий `nopal` — сам по себе повод
   идти вверх по цепочке.
...
`RIDDICK_SKIN_DRAWNOPALETTE=1` (вернуть старое поведение — артефакт
вернулся? значит это была причина №1), `RIDDICK_SKINNING=0` (весь
