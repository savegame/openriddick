...
* `M_NOEXCEPTIONINFO` не определён, поэтому `Error(...)` использует `this` —
  значит вызовы из свободных функций не скомпилировались бы. Скан по дереву
  дал 5 кандидатов, из них 3 — ложные (комментарии и функция с именем
  `GetDXT1Error`), а 2 настоящих — в `MInput_PS3.cpp`, который в
  Linux-сборку не входит.
