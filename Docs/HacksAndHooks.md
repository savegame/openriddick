...
- **DBG** `RIDDICK_DBG_MOVE=1` (`WObj_CharControlModeAnim.cpp`,
  ...
  `[MOVE] obj=… res=… anim=… real=… animV=(…) face= velang= rotw= pos=(…)`.
  ...
  Дополнено 2026-08-21 (охота за застывшей ходьбой): `face` — абсолютный
  угол фасинга, `velang` — абсолютный угол фактического курса (градусы,
  те же единицы, что `ang=` в трассе входа AG2), `rotw` — w-компонента
  RotVelocity, запрошенного анимацией (rotw≈1 — клип вращения не просит).
  Чтение при «застывшей ходьбе»: если `velang` уплывает от `face`, а
  `face` стоит на месте — тело не доворачивается за курсом, граф флапает
  состояниями направления (каждый вход сбрасывает время слоя).
...
## Лампы: световые текстуры, ореолы и конусы фонарей (исследование 2026-08-23)

Симптомы: (1) светотекстурные эффекты ламп не рисуются -- ореол фонаря,
тень решётки; (2) фонари NPC дублируются -- много ореолов вместо одного.
...
1. Повторный cookie NDSP -- известный открытый дефект (A/B:
   RIDDICK_NO_PROJ=1, RIDDICK_CUBE_ONEFACE, RIDDICK_PROJ_2D).
2. Портальные sub-view рисуют конус повторно без clip-плоскостей
   (CRC_FLAGS_CLIP в бэкенде не реализован; гейт GetVCDepth()==0 в
   WObj_CharRender.cpp:301-310 для A/B).
...
### Верификация texgen кук спотов (2026-08-23, численно по логу)

Прогон с RIDDICK_DBG_GL=1 на i1_showers дал фактические строки texgen
лампы Cube_Lamp008_00 (lightPos=(-1820,1188,36)):

    projU=(~0, -1, ~0, +1226)
    projV=(-1, ~0, ~0, -1820)
    projW=(~0, ~0, -1, +48)

Формула CreateProjMapTexGenAttr (XRShader_FP20.cpp:52-72): строка =
[ось_i | -dot(ось_i, Pos)], оси Y/X/Z делятся на SpotWidth/SpotHeight.
Числовая проверка: dot(U,P) при P=лампа = 1188/1226... т.е. в точке
лампы все три компоненты = 0; в произвольной точке P вектор =
относительный «P - лампа» в локальных осях света (с масштабами 1/W,
1/H). КУБИЧЕСКАЯ ВЫБОРКА использует только НАПРАВЛЕНИЕ вектора --
математика корректна. Вывод: если куки не видны -- проблема в
СОДЕРЖИМОМ загруженной кубической текстуры (одна картинка тиражируется
на 6 граней -- известный дефект) или её альфе, НЕ в texgen.

Дубли конусов фонарей NPC: конус добавляется при !bFirstPerson, в т.ч.
в портальных подвидах (GetVCDepth()>0); GLES3-бэкенд не потребляет
клип-плоскости порталов -> конус аддитивно дублируется. Краткосрочно
применён скип конуса при глубине>0 (WObj_CharRender.cpp:311); правильно
на M4+: gl_ClipDistance.
