...
## 4a. ВАЖНО: Riddick освещается динамически, не лайтмапами

Замечание пользователя (2026-07-27), проверено на оригинальной игре: **в самом
Riddick лайтмапов визуально нет — весь свет динамический** (движок из той же
эпохи и школы, что Doom 3: per-pixel источники + стенсильные тени). Механизм
лайтмап-запечки остался в коде наследием предыдущих игр Starbreeze и на картах
Riddick либо не используется, либо даёт слабую ambient-подложку.

Следствия для порта:
- слабый или нулевой визуальный вклад `XRShader_FP20_LFM` — **ожидаемый**
  результат, а не признак ошибки в реализации;
- реальный свет в кадре дают аддитивные per-light проходы
  `XRShader_FP20_NDS` / `_NDSP` (§4) — именно они приоритетны;
- это же объясняет чёрные стены до реализации FP20: если статического
  запечённого света в картах почти нет, то без per-light проходов мир и
  обязан быть тёмным.
