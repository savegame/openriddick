...
| 2026-07-27 | Стратегия рендера (`Docs/Render_Strategy.md`): градиенты на стенах = depth-fog проход без TexGen; движок имеет генератор VP по `CRC_VPFormat` + именованные FP20-программы (5 forward), полная реализация ≈3-5k строк; порядок работ R0..R7 | план |
| 2026-07-27 | Фаза 4 R0: GPU-кэш геометрии по VBID (`RenderContexts/GLES3/GLES3_Geometry.*`, `CGLES3GeometryCache`) — interleaved VBO + uint16 IBO строятся один раз через движковый `ConvertToInterleaved`, инвалидация по биту `CRC_VBIDInfo::m_Fresh` (`VB_MakeDirty`), реализованы `Geometry_Precache`/`Geometry_PrecacheFlush`, добавлен пропущенный `VB_Release`. Старый путь (malloc+VRegFetch+стриминг) остаётся как fallback и под `RIDDICK_NO_VBCACHE=1` | Фаза 4 |
| 2026-07-30 | Крэш в конце прогона i1_pigsville устранён: `Cluster_SetMatrixPalette` (WTriMesh.cpp) делал placement-new прямо на результат `CXR_VBManager::Alloc`, а тот на исчерпанной за кадр арене возвращает NULL -> конструктор `CRC_MatrixPalette` писал по адресу 0 (SIGSEGV в рендер-воркере); проверка `if(!pMP)` после placement-new мертва по построению. Первопричина переполнения арены — CPU-скиннинг (бэкенд не объявляет `CRC_CAPS_FLAGS_MATRIXPALETTE`): на каждый видимый кластер выделяется весь вершинный буфер дважды. Дефолт `XR_VBHEAP` под Linux 4096->32768 КиБ + `RIDDICK_VBHEAP=<KiB>`; учёт арены `[VBM] heap/used/demand/failed/VBs` (`RIDDICK_DBG_VBM=1`, при переполнении всегда), спам `Out of VB memory!` урезан до одной строки на кадр. Результаты зондов того же лога — `Docs/HacksAndHooks.md`, отчёты MoveSpeed/WeaponModel | bring-up |
| 2026-07-30 | Прогон без падения подтвердил арену (0 отказов VB), но картинка деградирует по ходу сессии — отдельный дефект. Три причины «вершина улетела за экран» в скиннинге: (1) скиннед-формат без палитры — разобрано по движку: у не анимированного кластера это штатная bind-поза (рисуем как раньше), фатально только для анимированного; добавлены счётчик `nopal=` и A/B-флаг `RIDDICK_SKIN_DROPNOPALETTE=1`, (2) индекс кости за пределами палитры читал uniform-хвост от ПРЕДЫДУЩЕГО draw'а (риги 70-120 костей против `GLES3_MAX_BONES=64`) — кламп в шейдере + обнуление и загрузка всех строк + безфлаговый `[GLES3-SKIN] palette OVER CAP`, (3) индексы за пределами вершинного массива на общем кольцевом VBO — `RIDDICK_IDXCHECK=1|2`. Счётчики `nopal/skinovercap/maxbones/idxbad` в `[GL-DBG]`; `[VBM] arena created` при старте; диагностика роста TArray дополнена elem/len/alloc/grow | bring-up |
| 2026-07-30 | Скиннинг: измерено, что GPU-путь не выполнялся ни разу (`skin=0` в 416 интервалах `[GL-DBG]`), а зонд `RIDDICK_DBG_PALETTE` дал `[MP] calls=74000 indirect=74013(max 48 bones) full=0` — все палитры кластер-локальные и ≤48 костей при потолке 64. Отсюда `RIDDICK_HWSKIN=1`: бэкенд объявляет `CRC_CAPS_FLAGS_MATRIXPALETTE`, движок перестаёт CPU-скиннить анимированные кластеры в VB-арену (`WTriMesh.cpp:5543`) и шлёт VBID + палитру на готовый GPU-путь; флаг подразумевает `RIDDICK_SKINNING=1` и в бэкенде, и в кэше геометрии. Ни компактизация костей, ни UBO, ни CPU-фолбэк не нужны. Проверка индексов (`RIDDICK_IDXCHECK=1`) дала `idxbad=0` — версия про выход индексов за границы закрыта | Фаза 4 |
