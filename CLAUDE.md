...
| 2026-07-14 | Фаза 4 M3: реальный draw path — единый GLSL ES 3.00 UI-шейдер (pos+uv+col+uUseTexture+uTex), интерлив вершин `SUIVert{xyz uv col=RGBA-swapped-BGRA}` из `m_Geom`, стриминг в кольцевой VBO/IBO, `glDrawElements`; `Render_IndexedTriangles/Strip/Wires/Polygon/Primitives` реализованы; текущая текстура берётся из `m_pCurAttrib->m_TextureID[0]` и лениво аплоадится; MVP = Model*Proj + column-major upload. `Render_VertexBuffer(VBID)` пока no-op. | Фаза 4 |
